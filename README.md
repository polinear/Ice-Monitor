...
docker compose exec -T superset python /opt/ice-monitor/scripts/bootstrap_superset_content.py
```

## План демо
1. Открыть `frontend` и показать live видео, маски и легенду.
2. Показать `api/docs` и `pipeline_status`, чтобы видно было состояние потоков и БД.
3. Показать `ingest` и `inference`, как кадры попадают в Redis и получают маски.
4. Показать Kafka и HDFS как холодный слой истории.
5. Показать Hive и Spark ETL.
6. Показать MLlib-эксперимент.
7. Показать Superset: `Модель и хранилище` и `Ледовая аналитика`.

## Примечания
- Судно не считается льдом и вырезается из рабочей области.
- Superset-дашборды создаются автоматически.
- Белая тема используется только в Superset.
- Ручная настройка чарта не нужна.
